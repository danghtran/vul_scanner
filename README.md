...
| **NVD enrichment** | **CPE `cpeName` lookup** for versioned stacks (OpenSSH, httpd, nginx, Tomcat, Postfix, vsftpd, …) with per-product cap/rank; **keyword** fallback; **disk cache** (`.cache/nvd`, 24h TTL) and retries on 429/5xx. HTTP port probes send **HTTP/1.1 `Host`** (virtual host) for better `Server` banners. |
